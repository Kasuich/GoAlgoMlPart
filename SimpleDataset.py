...
class SimpleDataset:
    @staticmethod
    def create_dataset(
        features, ticker, timeframe, candles=10000, notebook: bool = False
    ):
        raw_dataset = SimpleDataset.load_dataset(
            ticker, timeframe, candles=candles, notebook=notebook
        )
        raw_columns = list(raw_dataset.columns)
        SimpleDataset.make_dataset(raw_dataset, target_col="close")
        if features["lags"]:
            SimpleDataset.create_lags(
                raw_dataset, features["lags"]["features"], features["lags"]["period"]
            )
        if features["cma"]:
            SimpleDataset.create_cma(raw_dataset, features["cma"]["features"])
        if features["sma"]:
            SimpleDataset.create_sma(
                raw_dataset, features["sma"]["features"], features["sma"]["period"]
            )
        if features["ema"]:
            SimpleDataset.create_ema(
                raw_dataset, features["ema"]["features"], features["ema"]["period"]
            )
        if features["green_candles_ratio"]:
            SimpleDataset.add_green_ratio(
                raw_dataset, period=features["green_candles_ratio"]["period"]
            )
        if features["red_candles_ratio"]:
            SimpleDataset.add_red_ratio(
                raw_dataset, period=features["red_candles_ratio"]["period"]
            )
        if features["rsi"]:
    ...
    @staticmethod
    def load_dataset(
        ticker: str, timeframe: str, candles: int = 10000, notebook: bool = False
    ):
        ...
        if not notebook:
            ...
            def pandas_frame(candles_it: pd.DataFrame):
                pass
                return pd.DataFrame(
                    [normalize_row(row) for i, row in candles_it.iterrows()]
                )
        ...
        tick = Ticker(ticker)
        tradestats = pandas_frame(
            tick.candles(
                date=date_l.strftime("%Y-%m-%d"),
                till_date=date_r.strftime("%Y-%m-%d"),
                period=timeframe,
            )
        )
    ...
    @staticmethod
    def make_dataset(
        tradestats: pd.DataFrame, target_col: str = "close"
    ) -> pd.DataFrame:
    ...
    def create_sma(
        ...
        for feat in sma_feats:
            for size in sma_size:
                ...
                tradestats[agg_col] = (
                    tradestats[feat].rolling(size).mean()
                )  # среднее арифметическое значение отдельных периодов (минут, часов, дней и так далее) за выбранный промежуток времени
    ...
    def create_ema(
        ...
        for feat in ema_feats:
            for size in ema_size:
                ...
                tradestats[agg_col] = (
                    tradestats[feat].ewm(span=size).mean()
                )  # Экспоненциально взвешенное скользящее среднее

    @staticmethod
    def add_green_ratio(tradestats: pd.DataFrame, period: Iterable[int] = None):
        if period:
            ...
            for i in range(1, len(tradestats) + 1):
                green_candles[i] = (
                    green_candles[i - 1]
                    if razn.iloc[i - 1] < 0
                    else green_candles[i - 1] + 1
                )

            for n in period:
                agg_col = f"green_{n}"
                tradestats[agg_col] = [
                    np.nan if i < n else (green_candles[i] - green_candles[i - n]) / n
                    for i in range(len(tradestats))
                ]

    @staticmethod
    def add_red_ratio(tradestats: pd.DataFrame, period: Iterable[int] = None):
        if period:
            ...
            for n in period:
                ...
                tradestats[agg_col] = [
                    np.nan if i < n else (red_candles[i] - red_candles[i - n]) / n
                    for i in range(len(tradestats))
                ]

    @staticmethod
    def add_rsi(tradestats: pd.DataFrame, period: Iterable[int] = None):
        if period:
            for n in period:
                agg_col = f"rsi_{n}"
                rise = (tradestats["close"] - tradestats["open"]).apply(
                    lambda x: x if x > 0 else 0
                )
                fall = (tradestats["close"] - tradestats["open"]).apply(
                    lambda x: np.abs(x) if x < 0 else 0
                )
    ...
    @staticmethod
    def add_macd(tradestats: pd.DataFrame, period: Iterable[(int, int)] = None):
        if period:
            for n in period:
    ...
    def add_bollinger(
        ...
        if period and degree_of_lift:
            ...
            std_dev = [
                np.nan
                if i <= period
                else np.std(tradestats.iloc[i - period : i]["center_line"])
                for i in range(len(tradestats))
            ]
            tradestats["up_line"] = [
                tradestats.iloc[i]["center_line"] + degree_of_lift * std_dev[i]
                for i in range(len(tradestats))
            ]
            tradestats["down_line"] = [
                tradestats.iloc[i]["center_line"] - degree_of_lift * std_dev[i]
                for i in range(len(tradestats))
            ]
