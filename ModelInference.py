...
class ModelInference():
    ...
    def get_pred_one_candle(self,
                            seed=42,
                            candles=200,
                            date_col: str='date',
                            target_col: str='target'):
        model_path = f'{self.user_id}_{self.ticker}_{self.timeframe}_{self.features["model"]}.bin'
        test_data = SimpleDataset.create_dataset(features=self.features, ticker=self.ticker, timeframe=self.timeframe, candles=candles, notebook=self.notebook)
