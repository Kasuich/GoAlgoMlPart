...
class IfInference():
    ...
    def anomaly(
        ...
        count_df = SimpleDataset.create_dataset(features, ticker, timestamp, candles=10_000, notebook=notebook)
    ...
    def out_of_limits(
        df: pd.DataFrame,
        condition: tp.Literal["high", "low"],
        feature_name: tp.Literal["close", "high", "low", "open", "value", "volume", "green_candles_ratio", "red_candles_ratio", "price_changing"],
        limit: float,
    ...
    def average_cross(
        ...
        tmp_anomaly_df[signal_col] = delta.rolling(2).apply(lambda x: ((x.iloc[0] > 0) and (x.iloc[1] < 0)) * 1)
